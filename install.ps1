...
function Write-CyberFail {
    ...
    Write-BoxRow -Text "$cCross INSTALLATION FAILED" -Color $Theme.Fail
...
function Write-CyberSuccessCard {
    param([string]$TargetDir, [string]$Version, [string]$Elapsed)
    Write-Host ""
    Write-BoxTop -Color $Theme.Success
    Write-BoxRow -Text "$cCheck INSTALLATION SUCCESSFUL" -Color $Theme.Success
    Write-BoxDivider -Color $Theme.SuccessDim

    Format-BoxLine -Label "Target Directory : " -Value $TargetDir
    Format-BoxLine -Label "Installed Version: " -Value $Version
...
try {
    ...
    Write-CyberSuccessCard -TargetDir $InstallDir -Version $($Release.tag_name) -Elapsed $ElapsedStr
} catch {
